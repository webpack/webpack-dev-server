 ...
 * doubles as the feature probe for them. Either way the schema is only read,
 * and ajv only compiled, once something is actually wrong — the precompiled
 * validator answers the common case in ~1ms, against the ~120ms `schema-utils`
 * spends compiling the schema on its first call.
 ...
 * @returns {void}
 */
function validateOptions(compiler, options) {
  const firstCompiler =
    compiler && "compilers" in compiler ? compiler.compilers[0] : compiler;

  if (firstCompiler && firstCompiler.hooks.validate) {
    /** @type {EXPECTED_ANY} */
  ...
  // alone. It is also the path taken when no compiler was passed, which is how
  // the server is constructed for `apply()`.
