...
class Server {
  ...
  static addAdditionalEntries(compiler, options = {}) {
    ...
    if (options.client && isWebTarget(compiler.options)) {
      const getWebSocketURL = () => {
        ...
        else if (typeof options.host !== "undefined") {
      ...
      const webSocketURL = options.webSocketServer ? getWebSocketURL() : "";
  ...
  async initialize() {
    ...
    compilers.forEach((compiler) => {
      Server.addAdditionalEntries(compiler, this.options);

      const webpack = compiler.webpack || require("webpack");
      const getWebSocketTranport = () => {
      ...
      const providePlugin = new webpack.ProvidePlugin({
        __webpack_dev_server_client__: getWebSocketTranport(),
      });
