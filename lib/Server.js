...
class Server {
  ...
  setupHistoryApiFallbackFeature() {
    ...
    if (typeof historyApiFallback.verbose === 'undefined') {
    ...
    this.app.use(
      require('connect-history-api-fallback')({ logger, ...historyApiFallback })
    );
  }
  ...
  checkHeader(headers, headerToCheck) {
    ...
    if (this.options.allowedHosts.includes('all')) {
      return true;
    }
  ...
  watchFiles(watchPath, watchOptions) {
    ...
    if (this.options.liveReload) {
      watcher.on('change', (changedPath) => {
        this.sendMessage(this.sockets, 'static-changed', changedPath);
      });
    }
