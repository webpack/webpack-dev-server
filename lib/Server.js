...
function Server(compiler, options) {
	...
	const statusPlugin = (percent, message) => {
		this.sockWrite(this.sockets, "status-update", { percent: percent, msg: message });
	}
	...
	compiler.apply(new webpack.ProgressPlugin(function(percent, msg, addInfo) {
		percent = Math.floor(percent * 100);
		if(addInfo)
			msg = msg + " (" + addInfo + ")";
		statusPlugin(percent, msg);
	}));
...
Server.prototype.listen = function(port, hostname) {
	...
	sockServer.on("connection", (conn) => {
		...
		if(this.status)
			this.sockWrite([conn], "status", this.status);
