...
function Server(compiler, options) {
	...
	app.all("*", (req, res, next) => {
		if(this.checkHost(req.hostname))
			return next();
...
Server.prototype.checkHost = function(hostname) {
	...
	if(this.allowedHosts && this.allowedHosts.length) {
		for(let hostIdx = 0; hostIdx < this.allowedHosts.length; hostIdx++) {
			...
			const allowedHostname = url.parse("http://" + allowedHost).hostname;
			if(allowedHostname === hostname) return true;

			// support "." as a subdomain wildcard
			// e.g. ".example.com" will allow "example.com", "www.example.com", "subdomain.example.com", etc
			if(allowedHostname[0] === ".") {
				if(hostname === allowedHostname.substring(1)) return true; // "example.com"
				if(hostname.endsWith(allowedHostname)) return true; // "*.example.com"
			}
		}
	}
	...
	if(typeof this.publicHost === "string") {
		const publicHostname = url.parse("http://" + this.publicHost).hostname;
