...
class Server {
  ...
  setupMiddlewares() {
    ...
    if (this.options.historyApiFallback) {
      ...
      // Sync originalUrl with the rewritten url so that webpack-dev-middleware
      // (which reads originalUrl) can serve the correct file after the rewrite.
      middlewares.push({
        name: "connect-history-api-fallback-url-sync",
        /**
         * @param {Request} req request
         * @param {Response} res response
         * @param {NextFunction} next next function
