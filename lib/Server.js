...
function Server(compiler, options) {
	...
	var sslPaths = options.sslPaths || {};

	this.listeningApp = options.https
		? https.createServer({
				// using built-in self-signed certificate
				key: fs.readFileSync(options.keyPath || path.join(__dirname, "../ssl/server.key")),
				cert: fs.readFileSync(options.certPath || path.join(__dirname, "../ssl/server.crt")),
				ca: fs.readFileSync(options.caPath || path.join(__dirname, "../ssl/ca.crt")),
				requestCert: true,
