...
class Server {
  ...
  async setupWatchStaticFiles() {
    ...
    if (watchFiles.length > 0) {
      for (const item of watchFiles) {
        if (item.watch) {
          const ignoreOutputPath = this.getOutputPathMatcher(item.directory);

          await this.watchFiles(item.directory, {
            ...item.watch,
            ignored: [
              ...(typeof item.watch.ignored === "undefined"
                ? []
                : Array.isArray(item.watch.ignored)
                  ? item.watch.ignored
                  : [item.watch.ignored]),
              ...(ignoreOutputPath ? [ignoreOutputPath] : []),
            ],
          });
        }
      }
   ...
   */
  getOutputPathMatcher(directory) {
    const compilers =
      /** @type {MultiCompiler} */
      (this.compiler).compilers || [this.compiler];
    // `static.directory` is taken as given and may be relative, while
    // `outputPath` and the paths the watcher reports are absolute, so
    // everything is resolved before being compared
    const resolvedDirectory = path.resolve(directory);
    /**
     * @param {string} parent the directory to test against
     * @param {string} child the path that may sit below it
    ...
    const outputPaths = compilers
      .map((compiler) => compiler.outputPath)
      .filter(Boolean)
      .map((outputPath) => path.resolve(outputPath))
      .filter((outputPath) => isInside(resolvedDirectory, outputPath));
    ...
    return (targetPath) => {
      ...
      return outputPaths.some(
        (outputPath) =>
          resolvedTarget === outputPath || isInside(outputPath, resolvedTarget),
      );
    };
  }
