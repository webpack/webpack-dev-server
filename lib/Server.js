...
const CLIENT_EVENT = 'webpackDevServerClientData';
...
class Server {
  ...
  subscribeClientData(handle) {
    this.app.on(CLIENT_EVENT, handle);
    return () => this.app.off(CLIENT_EVENT, handle);
  }
  ...
  listen(port, hostname, fn) {
    ...
    const returnValue = this.listeningApp.listen(port, hostname, (err) => {
      ...
      socket.on('connection', (connection) => {
        ...
        connection.on('message', (data) => this.app.emit(CLIENT_EVENT, data));
