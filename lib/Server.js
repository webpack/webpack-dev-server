...
class Server {
  ...
  listen(port, hostname, fn) {
    ...
    if (this.options.ipcSocket) {
      const net = require('net');
      const fs = require('fs');

      this.server.on('error', (e1) => {
        if (e1.code === 'EADDRINUSE') {
          const clientSocket = new net.Socket();
          clientSocket.on('error', (e2) => {
            if (e2.code === 'ECONNREFUSED') {
              // No other server listening on this socket so it can be safely removed
              fs.unlinkSync(this.options.ipcSocket);
              this.server.listen(
                this.options.ipcSocket,
                this.options.host,
                (err) => {
                  if (err) throw err;
                }
          ...
          clientSocket.connect({ path: this.options.ipcSocket }, () => {
      ...
      return this.server.listen(
        ...
        (err1) => {
          ...
          fs.chmod(this.options.socket, READ_WRITE, (err2) => {
            if (err2) throw err2;
            this.logStatus();
          });
        }
