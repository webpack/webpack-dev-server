...
function Server(compiler, options) {
  ...
  app.get('/__webpack_dev_server__/live.bundle.js', (req, res) => {
    ...
    fs.createReadStream(path.join(__dirname, '../public/live.bundle.js')).pipe(res);
  });

  app.get('/__webpack_dev_server__/sockjs.bundle.js', (req, res) => {
    res.setHeader('Content-Type', 'application/javascript');
    fs.createReadStream(path.join(__dirname, '../public/sockjs.bundle.js')).pipe(res);
  });

  app.get('/webpack-dev-server.js', (req, res) => {
    res.setHeader('Content-Type', 'application/javascript');
    fs.createReadStream(path.join(__dirname, '../public/webpack-dev-server.js')).pipe(res);
  });
...
Server.prototype._sendStats = function (sockets, stats, force) {
  ...
  if (stats.errors.length > 0) {
    this.sockWrite(sockets, 'errors', stripAnsi(stats.errors));
  } else if (stats.warnings.length > 0) {
    this.sockWrite(sockets, 'warnings', stripAnsi(stats.warnings));
  } else {
