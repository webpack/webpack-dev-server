...
class Server {
  ...
  static validateOptions(compiler, options) {
    const owner = compiler ? Server.findDevServerCompiler(compiler) : undefined;

    if (owner && owner.hooks.validate) {
      /** @type {EXPECTED_ANY} */
      (owner).validate(
        () => schema,
        options,
    ...
    // alone. It is also the path taken when no compiler was passed, which is
    // how the server is constructed for `apply()`.
