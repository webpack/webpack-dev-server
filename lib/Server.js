...
/**
 * @typedef {{ path?: string | string[] | undefined, context?: string | string[] | HttpProxyMiddlewareOptionsFilter | undefined } & HttpProxyMiddlewareOptions} ProxyConfigArray
 */
 ...
 * @property {(devServer: TODO) => void} [onAfterSetupMiddleware]
 * @property {(devServer: TODO) => void} [onBeforeSetupMiddleware]
 * @property {(devServer: TODO) => void} [onListening]
 * @property {(middlewares: TODO[], devServer: TODO) => void} [setupMiddlewares]
 */
...
class Server {
  ...
  constructor(options = {}, compiler) {
    ...
    validate(/** @type {Schema} */ (schema), options, "webpack Dev Server");

    this.compiler = /** @type {Compiler | MultiCompiler} */ (compiler);
    this.logger =
      /** @type {ReturnType<Compiler["getInfrastructureLogger"]>}*/
      (this.compiler.getInfrastructureLogger("webpack-dev-server"));
  ...
  getCompilerOptions() {
    if (
      ...
      if (/** @type {MultiCompiler} */ (this.compiler).compilers.length === 1) {
        return /** @type {MultiCompiler} */ (this.compiler).compilers[0]
          .options;
      }
  ...
  async normalizeOptions() {
    ...
    if (options.server.type === "https" || options.server.type === "spdy") {
      ...
      if (/** @type {ServerOptions} */ (options.server.options).cacert) {
  ...
  createServer() {
    ...
    this.server = require(this.options.server.type).createServer(
      /** @type {ServerConfiguration} */ (this.options.server).options,
      this.app
    );
  ...
  createWebSocketServer() {
    ...
    this.webSocketServer.implementation.on("connection", (client, request) => {
      ...
      if (
        this.options.client &&
        /** @type {ClientConfiguration} */ (this.options.client).overlay
      ) {
        this.sendMessage(
          [client],
          "overlay",
