...
function Server(compiler, options) {
	...
	app.get("/webpack-dev-server", (req, res) => {
		...
		if(Array.isArray(options.publicPath)) {
			publicPathArray = options.publicPath;
		} else {
			publicPathArray = [options.publicPath];
		}
		const fs = this.middleware.fileSystem;

		function writeDirectory(baseUrl, basePath, baseUrlTitle) {
		...
		const baseUrlTitle = publicPathArray.length > 1;
		publicPathArray.forEach(function(publicPath) {
			const path = middleware.getFilenameFromUrl(publicPath || "/");
			writeDirectory(publicPath || "/", path, baseUrlTitle);
		});
