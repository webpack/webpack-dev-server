...
const addProgressPlugin = require('./utils/addProgressPlugin');
const checkHost = require('./utils/checkHost');
const createLogger = require('./utils/createLogger');
const getSSLCertificate = require('./utils/getSSLCertificate');
const statsOptions = require('./utils/statsOptions');
...
function Server (compiler, options = {}, _log) {
  ...
  // Require for `checkHost`, remove in next major release
  ...
  if (this.progress) {
    addProgressPlugin(compiler, this);
  }
  ...
  const addHooks = (compiler) => {
    ...
    invalid.tap('webpack-dev-server', invalidPlugin);
      ...
      this._sendStats(this.sockets, stats.toJson(statsOptions));
      this._stats = stats;
    });
  };
  ...
  this.middleware = webpackDevMiddleware(
    compiler,
    Object.assign({}, options, { logLevel: this.log.options.level })
  );
  ...
  if (options.https) {
    ...
    if (!options.https.key || !options.https.cert) {
      fakeCert = getSSLCertificate(this);
    }
...
Server.prototype.listen = function (port, hostname, fn) {
  ...
  return this.listeningApp.listen(port, hostname, (err) => {
    ...
    socket.on('connection', (connection) => {
      ...
      this._sendStats([ connection ], this._stats.toJson(statsOptions), true);
    });
...
// Remove from public api in next major release
Server.prototype.checkHost = checkHost;
