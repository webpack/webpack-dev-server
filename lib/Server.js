...
class Server {
  ...
  getOutputPathMatcher(directory) {
    const compilers =
      /** @type {MultiCompiler} */
      (this.compiler).compilers || [this.compiler];
    // a path already ending in the separator is a filesystem root, and
    // appending a second one would match nothing below it
    const asPrefix = (/** @type {string} */ aPath) =>
      aPath.endsWith(path.sep) ? aPath : aPath + path.sep;
    const directoryPrefix = asPrefix(directory);
    const prefixes = compilers
      .map((compiler) => compiler.outputPath)
      .filter(
        (outputPath) =>
          outputPath &&
          outputPath !== directory &&
          outputPath.startsWith(directoryPrefix),
      )
      .map((outputPath) => asPrefix(outputPath));
    ...
    return (targetPath) =>
      prefixes.some(
        (prefix) =>
          targetPath === prefix.slice(0, -path.sep.length) ||
