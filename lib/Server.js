...
class Server {
  ...
  createWebSocketServer() {
    ...
    this.webSocketServer.onConnection((connection, headers) => {
      ...
      this.webSocketServer.clients.push(connection);

      this.webSocketServer.onConnectionClose(connection, () => {
        const idx = this.webSocketServer.clients.indexOf(connection);

        if (idx >= 0) {
          this.webSocketServer.clients.splice(idx, 1);
        }
      });
  ...
  sendMessage(clients, type, data) {
    clients.forEach((webSocketConnection) => {
