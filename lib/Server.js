...
class Server {
  ...
  async setupWatchStaticFiles() {
    ...
    if (watchFiles.length > 0) {
      const ignoreOutputPath = this.getOutputPathMatcher();
  ...
  getOutputPathMatcher() {
    ...
    // an output path already ending in the separator is a filesystem root, and
    // appending a second one would match nothing below it
    const prefixes = outputPaths.map((outputPath) =>
      outputPath.endsWith(path.sep) ? outputPath : outputPath + path.sep,
    );

    return (targetPath) =>
      outputPaths.some((outputPath, index) =>
        targetPath === outputPath
          ? true
          : targetPath.startsWith(prefixes[index]),
      );
  }
