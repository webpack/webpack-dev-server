...
class Server {
  ...
  showStatus() {
    const useColors = this.options.stats && this.options.stats.colors;

    if (this.options.socket) {
      this.log.info(
        `Listening to socket at ${colors.info(useColors, this.options.socket)}`
      );
    } else {
      this.log.info(
        `Project is running at ${colors.info(useColors, this.url)}`
      );
    }
    ...
    const contentBase = Array.isArray(this.options.contentBase)
      ? this.options.contentBase.join(', ')
      : this.options.contentBase;
  ...
  listen(port, hostname, fn) {
    ...
    return this.listeningApp.listen(port, hostname, (err) => {
      ...
      this.url = `${createDomain(this.options, this.listeningApp)}${suffix}`;
      ...
      if (this.options.open) {
        runOpen(this.url, this.options, this.log);
      }
