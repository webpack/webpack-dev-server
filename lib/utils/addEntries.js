 ...
 * @returns {void}
 */
function addEntries(compilers, config, options, server) {
  ...
  /**
   * getInjectedEntries Method
   * @param {Entry} originalEntry
   * @param {Entry} additionalEntries
   * @returns {Entry}
   */
  const getInjectedEntries = (originalEntry, additionalEntries) => {
    if (typeof originalEntry === 'function') {
      return () =>
        Promise.resolve(originalEntry()).then((entry) =>
          getInjectedEntries(entry, additionalEntries)
        );
    }
    ...
    if (typeof originalEntry === 'object' && !Array.isArray(originalEntry)) {
      ...
      Object.keys(originalEntry).forEach((key) => {
        ...
        if (typeof entryDescription === 'object' && entryDescription.import) {
          entryObj[key] = Object.assign({}, entryDescription, {
            import: additionalEntries,
          });
        } else {
          entryObj[key] = additionalEntries;
        }
      });
  ...
  [].concat(config).forEach((config) => {
    ...
    const injectedEntries = getInjectedEntries(
      config.entry || './src',
      additionalEntries
    );
    console.log(injectedEntries);
    compilers.forEach((compiler) => {
      const compilerConfig = compiler.options;
      compiler.hooks.entryOption.call(compilerConfig.context, injectedEntries);
    });
