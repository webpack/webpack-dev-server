...
class DevServerPlugin {
  ...
  apply(compiler) {
    ...
    if (typeof options.webSocketURL === 'string') {
    ...
    } else if (typeof options.webSocketURL === 'object') {
      /** @type {string} */
      let host = '';

      if (options.webSocketURL.host) {
        host = `&host=${options.webSocketURL.host}`;
      ...
      if (options.webSocketURL.port) {
        port = `&port=${options.webSocketURL.port}`;
      ...
      if (options.webSocketURL.path && options.webSocketURL.path) {
        path = `&path=${options.webSocketURL.path}`;
      } else if (options.webSocketServer.options.path) {
        path = `&path=${options.webSocketServer.options.path}`;
      }

      webSocketURL = `${host}${path}${port}`;
    }
