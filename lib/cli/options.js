...
function parseFlags(argv, devServerOptions) {
  const result = {};
  ...
  if (argv.host !== 'localhost' || !devServerOptions.host) {
...
module.exports = function options(argv, webpackOptions) {
  ...
  const result = Object.assign(base, copyFlags(argv), parseFlags(argv));
